...
public class ArbolAVL<E> extends ÁrbolBinario{
    ...
    public void equilibrarArbol(NodoDeArbol<E> nodo, NodoDeArbol<E> padreDeNodo, String tipoHijo){//el null nunca se utilizará porque en la raíz al revisar el FE, nunca se encontrará un FE anormal, porque el desequilibrio se habrá arreglado (Si es que eixstiera xD) antes de llegar ahi xD
        if(nodo != null){
            equilibrarArbol(nodo.darHijoDerecho(), nodo, "derecho");//Creo que al fianl de cuentas esto no da problema, ademśa creo que es la única manera de revisar los hijos del nodo en cuestión de manera correcta... o creo que lo puse así porque tenía que ver con la reglamnetación para hacer el reequilivrio :v, pero sí funciona xD, CREO... XD
            equilibrarArbol(nodo.darHijoIzquierdo(), nodo, "izquierdo");
    ...
    private void hallarTipoRotacion(int factorDeEquilibrio, NodoDeArbol<E> nodo, NodoDeArbol<E> nodoPadre, String tipoHijo){//este tipo de hijo es del nodo con FE = |2|                        
    ...
    private void rotarSimplementeALaIzquierda(NodoDeArbol<E> nodoDesequilibrado, NodoDeArbol<E> padreDelDesequilibrado, String tipoHijo){
        ...
        switch (tipoHijo) {//mejor deje las condiciones aquí porque si las coloco en un método que me permita evitar tenerlo en ambos métodos [para rotIzq y rotDer simples] aún así tendría que colocar otros 2 para cuando fueran doble y al hacer eso terminaría con más bloques de estos [específicamente 1 más es decir:3...] que al dejarlos aquí 
            case "derecho":
                ...
                break;
            case "izquierdo":
                padreDelDesequilibrado.reestablecerHijoIzquierdo(nodoAuxiliar);
                break;
    ...
    private void rotarSimplementeALaDerecha(NodoDeArbol<E> nodoDesequilibrado, NodoDeArbol<E> padreDelDesequilibrado, String tipoHijo){
    ...
    private void rotarDoblementeALaIzquierda(NodoDeArbol<E> nodoDesequilibrado, NodoDeArbol<E> padreDelDesequilibrado, String tipoHijo){//este tipo de hijo es del desequilibrado, es decir del que tiene FE = |2|
        //este orden es así, pues se debe arreglar primero el hijo y luego al padre 
        rotarSimplementeALaIzquierda(nodoDesequilibrado.darHijoIzquierdo(), nodoDesequilibrado, "izquierdo");//Se supone que se deberí aactualizar el mero obj al salir de este proceso ;;        
    ...
    private void rotarDoblementeALaDerecha(NodoDeArbol<E> nodoDesequilibrado, NodoDeArbol<E> padreDelDesequilibrado, String tipoHijo){
        rotarSimplementeALaDerecha(nodoDesequilibrado.darHijoDerecho(), nodoDesequilibrado, "derecho");
